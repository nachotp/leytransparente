...
query = mycol.find(
    ...
    {"Id_Declaracion": 1, "Datos_del_Declarante": 1,
     "Derechos_Acciones_Chile": {"$elemMatch": {"Giro_Registrado_SII": {"$in": kwrds}}}}
)

matches = []
for weon in query:
    nombre = weon["Datos_del_Declarante"]["Apellido_Paterno"] + " " + weon["Datos_del_Declarante"][
        "Apellido_Materno"] + " " + weon["Datos_del_Declarante"]["nombre"]
